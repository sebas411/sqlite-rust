...
pub fn get_table_info(buffer: &[u8]) -> Vec<Table> {
...
pub fn read_cell(file: &File, page_num: u32, page_size: usize, where_expr: &Option<Expr>, available_columns: &Vec<Column>, print_columns: &Vec<usize>) -> Result<()> {
    ...
    if page_type == 13 {
        ...
        for i in 0..cell_num as usize {
            ...
            get_varint(&buffer, &mut current_offset); // size of record
            let rowid = get_varint(&buffer, &mut current_offset); // the rowid
    ...
    // interior page
        ...
        for i in 0..cell_num as usize {
            let current_offset = u16::from_be_bytes([buffer[12+2*i], buffer[12+2*i+1]]) as usize;
            ...
            read_cell(file, cell_page, page_size, where_expr, available_columns, print_columns)?;
        }
        read_cell(file, last_page, page_size, where_expr, available_columns, print_columns)?;
    } else {
